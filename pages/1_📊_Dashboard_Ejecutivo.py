...
def cargar_conversaciones_desde_google_sheets():
    ...
    # Renombrar columnas (ajusta nombres exactos)
    df = df.rename(
        columns={
            "Fecha": "fecha",
            "Nombre": "usuario",
    ...
    # Convertir fecha - CLAVE: usar dayfirst=True para formato DD/MM/YYYY
    df["fecha"] = pd.to_datetime(df["fecha"], dayfirst=True, errors="coerce")
    
    # Mostrar cuántas fechas son inválidas ANTES de dropna
    fechas_invalidas = df["fecha"].isna().sum()
    if fechas_invalidas > 0:
        st.sidebar.warning(f"⚠️ {fechas_invalidas} filas con fechas inválidas")
    
    # Eliminar solo filas sin fecha válida
...
# Mostrar estado de carga en sidebar (SIN el debug de columnas)
with st.sidebar:
