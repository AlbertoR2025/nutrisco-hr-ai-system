...
class ChatDatabase:
    ...
    def guardar_conversacion(self, usuario, consulta, respuesta, 
                            categoria="General", area=None, 
                            tiempo_respuesta_mins=None, derivado=False):
    ...
    def buscar_conversaciones(self, query_texto=None, fecha_desde=None, 
                             fecha_hasta=None, categoria=None, 
                             area=None, derivado=None):
    ...
    def registrar_busqueda(self, usuario_buscador, query_busqueda, 
                          filtros=None, resultado_encontrado=True):
    ...
    def calcular_kpis(self, fecha_desde=None, fecha_hasta=None):
        ...
        # Total consultas
        query_total = f"SELECT COUNT(*) as total FROM conversaciones WHERE {conditions}"
        ...
        '''
        derivados = pd.read_sql_query(query_derivados, conn, params=params)['derivados'].iloc[0]
        ...
        '''
        tiempo_prom = pd.read_sql_query(query_tiempo, conn, params=params)['tiempo_promedio'].iloc[0]
        ...
        '''
        temas_nuevos = pd.read_sql_query(query_emergentes, conn, params=[fecha_7dias])['temas_nuevos'].iloc[0]
